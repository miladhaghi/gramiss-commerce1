...
TARGETS={
113:{'title':'تیشرت باکسیه سنگشور دو تیکه طرح مسیح','excerpt':'تیشرت باکسی سنگشور دو تکه طرح مسیح با یقه‌گرد و طراحی چاپی، در سایزهای M، L و XL ارائه می‌شود.'},
119:{'title':'تیشرت باکسی سنگشور طرح closer','excerpt':'تیشرت باکسی سنگشور طرح closer با یقه‌گرد و طراحی چاپی، در رنگ‌های ذغالی و طوسی و سایزهای M و L ارائه می‌شود.'},
150:{'title':'تیشرت باکسی سنگشور طرح A.D.F','excerpt':'تیشرت باکسی سنگشور طرح A.D.F با یقه‌گرد و طراحی چاپی، در رنگ‌های سرمه‌ای و سفید و سایزهای M و L ارائه می‌شود.'},
175:{'title':'تیشرت باکس طرح Chanyoou','excerpt':'تیشرت باکس طرح Chanyoou با یقه‌گرد و طراحی چاپی، در سایزهای M، L و XL ارائه می‌شود.'},
181:{'title':'تیشرت سنگشور باکس طرح Warnning','excerpt':'تیشرت سنگشور باکس طرح Warnning با یقه‌گرد و طراحی چاپی، در رنگ‌های سبز و سرمه‌ای و سایزهای M، L و XL ارائه می‌شود.'},
}
def safe(u):
...
def get(u,timeout=150):
 ...
 for n in range(5):
  try:
   r=urllib.request.Request(u,headers={'User-Agent':'GramissLegacyShortDescBatch2/1.0','Cache-Control':'no-cache','Pragma':'no-cache'})
...
def php_payload(mode,old=None):
 targets=json.dumps({str(k):v for k,v in TARGETS.items()},ensure_ascii=False,separators=(',',':'));oldj=json.dumps(old or {},ensure_ascii=False,separators=(',',':'))
 return r'''<?php header('Content-Type: application/json; charset=utf-8');define('WP_USE_THEMES',false);require __DIR__.'/wp-load.php';@unlink(__FILE__);$mode='''+json.dumps(mode)+r''';$targets=json_decode('''+json.dumps(targets)+r''',true);$old=json_decode('''+json.dumps(oldj)+r''',true);$o=['ok'=>true,'errors'=>[],'before'=>[],'after'=>[]];foreach($targets as $sid=>$cfg){$id=(int)$sid;$p=get_post($id);$wc=wc_get_product($id);if(!$p||!$wc){$o['errors'][]="missing:$id";continue;}$vars=[];if($wc->is_type('variable'))foreach($wc->get_children() as $vid){$v=wc_get_product($vid);if($v)$vars[]=['id'=>(int)$vid,'sku'=>$v->get_sku(),'price'=>$v->get_price(),'stock_status'=>$v->get_stock_status()];}$o['before'][$sid]=['status'=>$p->post_status,'title'=>$p->post_title,'excerpt'=>$p->post_excerpt,'url'=>get_permalink($id),'vars'=>$vars];if($mode==='apply'){if($p->post_status!=='publish')$o['errors'][]="status:$id";if($p->post_title!==$cfg['title'])$o['errors'][]="title:$id";if(trim((string)$p->post_excerpt)!=='')$o['errors'][]="excerpt:$id";foreach($vars as $v){if(trim((string)$v['sku'])==='')$o['errors'][]='sku:'.$v['id'];if(trim((string)$v['price'])==='')$o['errors'][]='price:'.$v['id'];}}}if($mode==='apply'&&empty($o['errors']))foreach($targets as $sid=>$cfg){$r=wp_update_post(['ID'=>(int)$sid,'post_excerpt'=>$cfg['excerpt']],true);if(is_wp_error($r))$o['errors'][]='update:'.$sid.':'.$r->get_error_message();}if($mode==='rollback')foreach($targets as $sid=>$cfg){$r=wp_update_post(['ID'=>(int)$sid,'post_excerpt'=>array_key_exists($sid,$old)?(string)$old[$sid]:''],true);if(is_wp_error($r))$o['errors'][]='rollback:'.$sid;}foreach($targets as $sid=>$cfg){$p=get_post((int)$sid);if($p)$o['after'][$sid]=['title'=>$p->post_title,'excerpt'=>$p->post_excerpt,'url'=>get_permalink((int)$sid)];}if(function_exists('wp_cache_flush'))wp_cache_flush();do_action('litespeed_purge_all');do_action('rank_math/sitemap/flush_cache');$o['ok']=empty($o['errors']);echo wp_json_encode($o,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);?>'''
def call(mode,old=None):
 n='gramiss-shortdesc-b2-'+mode+'-'+hashlib.sha256((str(time.time())+mode).encode()).hexdigest()[:12]+'.php';save(n,php_payload(mode,old));s,b,f=get(BASE+'/'+n+'?t='+str(int(time.time())),240);print('WP_CALL',mode,s,f);d=json.loads(b.decode('utf-8','replace'));print('WP_RESULT',json.dumps(d,ensure_ascii=False,separators=(',',':')));
 if s!=200 or not d.get('ok'):raise RuntimeError(mode+' failed '+str(d.get('errors')))
 return d
def public_verify(urls):
 errs=[]
 for pid,cfg in TARGETS.items():
  u=urls[str(pid)];last={}
  for a in range(6):
   st,raw,_=get(u+'?t='+str(int(time.time())));txt=raw.decode('utf-8','replace');head=txt.split('</head>',1)[0];last={'status':st,'description':val(head,r'<meta[^>]+name=["\']description["\'][^>]+content=["\']([^"\']*)'),'canonical':val(head,r'<link[^>]+rel=["\']canonical["\'][^>]+href=["\']([^"\']+)'),'robots':val(head,r'<meta[^>]+name=["\']robots["\'][^>]+content=["\']([^"\']+)'),'schema':len(re.findall(r'"@type"\s*:\s*"Product"',txt,re.I))}
   if st==200 and last['description']==cfg['excerpt'] and norm(last['canonical'])==norm(u) and 'index' in last['robots'].lower() and 'noindex' not in last['robots'].lower() and last['schema']==1:break
   time.sleep(3+a*2)
  print('PUBLIC',pid,json.dumps(last,ensure_ascii=False,separators=(',',':')))
  if last.get('status')!=200:errs.append(f'{pid}:http')
  if last.get('description')!=cfg['excerpt']:errs.append(f'{pid}:meta')
  if norm(last.get('canonical',''))!=norm(u):errs.append(f'{pid}:canonical')
  if 'noindex' in last.get('robots','').lower() or 'index' not in last.get('robots','').lower():errs.append(f'{pid}:robots')
  if last.get('schema')!=1:errs.append(f'{pid}:schema')
 return errs
...
if ps!=200 or len(pu)!=47 or ph!=PRODUCT_SHA:raise SystemExit('product sitemap drift')
if cs!=200 or len(cu)!=20 or ch!=CAT_SHA:raise SystemExit('cat sitemap drift')
preflight=call('preflight');old={k:v['excerpt'] for k,v in preflight['before'].items()};urls={k:v['url'] for k,v in preflight['before'].items()}
for k,v in old.items():
 if v!='':raise SystemExit('excerpt not empty '+k)
applied=False
try:
 out=call('apply');applied=True
 for k,cfg in {str(k):v for k,v in TARGETS.items()}.items():
  if out['after'][k]['excerpt']!=cfg['excerpt']:raise RuntimeError('stored mismatch '+k)
 errs=public_verify(urls);ps2,pu2,ph2=sitemap('product-sitemap.xml');cs2,cu2,ch2=sitemap('product_cat-sitemap.xml');print('SITEMAP_POST',ps2,len(pu2),ph2,cs2,len(cu2),ch2)
 if pu2!=pu or ph2!=ph:raise RuntimeError('product sitemap changed')
 if cu2!=cu or ch2!=ch:raise RuntimeError('cat sitemap changed')
 post=hashes();print('PROTECTED_POST',json.dumps(post,sort_keys=True))
 if post!=pre:raise RuntimeError('protected changed')
 if errs:raise RuntimeError('public '+','.join(errs))
 print('PASS LEGACY SHORT DESCRIPTION BATCH 2')
