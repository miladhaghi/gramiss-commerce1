...
if 'GRAMISS_CHECKOUT_MOBILE_V1' not in css or 'GRAMISS_CHECKOUT_MOBILE_V1' not in js: raise SystemExit('ABORT: candidate assets invalid')
...
try:
    ...
    for path in ('assets/css/checkout-mobile-v1.css','assets/js/checkout-mobile-v1.js'):
        ...
        ok=status==200 and b'GRAMISS_CHECKOUT_MOBILE_V1' in body and len(body)>1000; print(('PASS' if ok else 'FAIL'),path,status,len(body))
