...
def get(u,follow=True,timeout=120):
 req=urllib.request.Request(u,headers={'User-Agent':'GramissSEOIndexationV1/1.1','Cache-Control':'no-cache','Pragma':'no-cache'})
...
if(class_exists('RankMath\\Sitemap\\Cache')){\RankMath\Sitemap\Cache::invalidate_storage();}$cache_after=get_option('rank_math_sitemap_cache_files',[]);global $wp_rewrite;if($wp_rewrite)$wp_rewrite->flush_rules(false);do_action('litespeed_purge_all');
echo wp_json_encode(['ok'=>true,'manifest'=>$mp,'cache_after'=>$cache_after,'sitemap'=>['pt_post_sitemap'=>$sm['pt_post_sitemap'],'pt_page_sitemap'=>$sm['pt_page_sitemap'],'pt_product_sitemap'=>$sm['pt_product_sitemap'],'tax_category_sitemap'=>$sm['tax_category_sitemap'],'tax_product_cat_sitemap'=>$sm['tax_product_cat_sitemap'],'authors_sitemap'=>$sm['authors_sitemap'],'include_featured_image'=>$sm['include_featured_image']],'noindex_ids'=>$ids],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
'''
...
for child in children:
 cs,cr,cu,_=get(child+'?t='+str(int(time.time())),True,120);ct=cr.decode('utf-8','replace');locs=re.findall(r'<loc>(.*?)</loc>',ct,re.I);all_urls+=locs;print('CHILD',child,'STATUS',cs,'COUNT',len(locs),'SAMPLE',json.dumps(locs[:5],ensure_ascii=False))
...
if errors:
 ...
 rbphp="<?php define('WP_USE_THEMES',false);require __DIR__.'/wp-load.php';@unlink(__FILE__);$m=json_decode(file_get_contents("+mp+"),true);update_option($m['sitemap_option_name'],$m['old_sitemap'],false);update_option($m['titles_option_name'],$m['old_titles'],false);foreach($m['old_post_robots'] as $id=>$r){if(!$r['exists'])delete_post_meta((int)$id,'rank_math_robots');else update_post_meta((int)$id,'rank_math_robots',$r['value']);}if(class_exists('RankMath\\Sitemap\\Cache')){\\RankMath\\Sitemap\\Cache::invalidate_storage();}global $wp_rewrite;if($wp_rewrite)$wp_rewrite->flush_rules(false);do_action('litespeed_purge_all');echo 'ROLLED_BACK';"
