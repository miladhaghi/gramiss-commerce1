...
TARGETS={296:{'title':'پیراهن آستین بلند پارچه سیلک هلویی و آبی‌طوسی','excerpt':'پیراهن آستین بلند پارچه سیلک هلویی و آبی‌طوسی؛ گزینه‌های رنگ ثبت‌شده آبی آسمانی و نارنجی هستند و سایزهای M، L، XL و 2XL برای این مدل تعریف شده است.'},307:{'title':'پیراهن آستین بلند پارچه سیلک قهوه‌ای و کرم','excerpt':'پیراهن آستین بلند پارچه سیلک قهوه‌ای و کرم؛ برای این مدل رنگ‌های قهوه‌ای و کرم و سایزهای L، XL و 2XL تعریف شده است.'},320:{'title':'پیراهن آستین بلند پارچه سیلک گرم‌دار','excerpt':'پیراهن آستین بلند پارچه سیلک گرم‌دار؛ برای این مدل سایزهای XL و 2XL تعریف شده است.'},325:{'title':'پیراهن آستین کوتاه کرپ','excerpt':'پیراهن آستین کوتاه کرپ؛ برای این مدل سایزهای M، L، XL و 2XL تعریف شده است.'},330:{'title':'پیراهن لینن آستین کوتاه سرمه‌ای','excerpt':'پیراهن لینن آستین کوتاه سرمه‌ای؛ برای این مدل سایزهای M و XL تعریف شده است.'}}
def safe(u):
...
def get(u,timeout=150):
 ...
 for n in range(5):
  try:
   r=urllib.request.Request(u,headers={'User-Agent':'GramissLegacyShirtsBatch7/1.0','Cache-Control':'no-cache','Pragma':'no-cache'})
...
def php(mode,old=None):
 ...
 return r'''<?php header('Content-Type: application/json; charset=utf-8');define('WP_USE_THEMES',false);require __DIR__.'/wp-load.php';@unlink(__FILE__);$mode='''+json.dumps(mode)+r''';$targets=json_decode('''+json.dumps(tj)+r''',true);$old=json_decode('''+json.dumps(oj)+r''',true);$o=['ok'=>true,'errors'=>[],'before'=>[],'after'=>[]];foreach($targets as $sid=>$cfg){$id=(int)$sid;$p=get_post($id);$wc=wc_get_product($id);if(!$p||!$wc){$o['errors'][]='missing:'.$id;continue;}$robots=get_post_meta($id,'rank_math_robots',true);$cats=wp_get_post_terms($id,'product_cat',['fields'=>'names']);$vars=[];if($wc->is_type('variable'))foreach($wc->get_children() as $vid){$v=wc_get_product($vid);if($v)$vars[]=['id'=>(int)$vid,'sku'=>$v->get_sku(),'price'=>$v->get_price(),'stock_status'=>$v->get_stock_status()];}$o['before'][$sid]=['status'=>$p->post_status,'title'=>$p->post_title,'excerpt'=>$p->post_excerpt,'url'=>get_permalink($id),'cats'=>$cats,'robots'=>$robots,'vars'=>$vars];if($mode==='apply'){if($p->post_status!=='publish')$o['errors'][]='status:'.$id;if($p->post_title!==$cfg['title'])$o['errors'][]='title:'.$id;if(trim((string)$p->post_excerpt)!=='')$o['errors'][]='excerpt:'.$id;if(!in_array('پیراهن',$cats,true))$o['errors'][]='family:'.$id;if(is_array($robots)&&in_array('noindex',$robots,true))$o['errors'][]='noindex:'.$id;foreach($vars as $v){if(trim((string)$v['sku'])==='')$o['errors'][]='sku:'.$v['id'];if(trim((string)$v['price'])==='')$o['errors'][]='price:'.$v['id'];}}}if($mode==='apply'&&empty($o['errors']))foreach($targets as $sid=>$cfg){$r=wp_update_post(['ID'=>(int)$sid,'post_excerpt'=>$cfg['excerpt']],true);if(is_wp_error($r))$o['errors'][]='update:'.$sid;}if($mode==='rollback')foreach($targets as $sid=>$cfg){$r=wp_update_post(['ID'=>(int)$sid,'post_excerpt'=>array_key_exists($sid,$old)?(string)$old[$sid]:''],true);if(is_wp_error($r))$o['errors'][]='rollback:'.$sid;}foreach($targets as $sid=>$cfg){$p=get_post((int)$sid);if($p)$o['after'][$sid]=['excerpt'=>$p->post_excerpt,'url'=>get_permalink((int)$sid)];}if(function_exists('wp_cache_flush'))wp_cache_flush();do_action('litespeed_purge_all');do_action('rank_math/sitemap/flush_cache');$o['ok']=empty($o['errors']);echo wp_json_encode($o,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);?>'''
def call(mode,old=None):
 n='gramiss-shirts-b7-'+mode+'-'+hashlib.sha256((mode+str(time.time())).encode()).hexdigest()[:12]+'.php';save(n,php(mode,old));s,b,f=get(BASE+'/'+n+'?t='+str(int(time.time())),240);d=json.loads(b.decode('utf-8','replace'));print('WP',mode,s,f,json.dumps(d,ensure_ascii=False,separators=(',',':')))
...
try:
 ...
 print('PASS LEGACY SHIRTS SHORT DESCRIPTION BATCH 7')
