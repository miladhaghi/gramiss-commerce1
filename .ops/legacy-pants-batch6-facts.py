...
nonce=hashlib.sha256((str(time.time())+pre['front-page.php']).encode()).hexdigest()[:12];name='gramiss-pants-b6-facts-'+nonce+'.php';ids=','.join(map(str,TARGETS))
...
save(name,php);req=urllib.request.Request(BASE+'/'+name+'?t='+str(int(time.time())),headers={'User-Agent':'GramissPantsBatch6Facts/1.0'});raw=urllib.request.urlopen(req,context=CTX,timeout=240).read();d=json.loads(raw.decode('utf-8','replace'));print('FACTS',json.dumps(d,ensure_ascii=False,separators=(',',':')));errs=list(d.get('errors') or [])
...
print('PASS LEGACY PANTS BATCH 6 FACTS')