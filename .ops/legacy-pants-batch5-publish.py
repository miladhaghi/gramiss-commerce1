...
TARGETS={260:{'title':'شلوار جین بالنی زاپدار','excerpt':'شلوار جین بالنی زاپ‌دار؛ برای این مدل سایزهای M، L و 2XL تعریف شده است.'},268:{'title':'شلوار بگ افه دار ذغالی','excerpt':'شلوار بگ افه‌دار ذغالی؛ برای این مدل سایزهای M، L و 2XL تعریف شده است.'},284:{'title':'شلوار بگ ذغالی طرحدار','excerpt':'شلوار بگ ذغالی طرح‌دار؛ برای این مدل سایزهای L، XL و 2XL تعریف شده است.'},288:{'title':'شلوار جین بگ دو تکه','excerpt':'شلوار جین بگ دو تکه؛ برای این مدل سایزهای M، L، XL و 2XL تعریف شده است.'},293:{'title':'شلوار جین کارگو زاپ','excerpt':'شلوار جین کارگو زاپ؛ برای این مدل سایزهای M و XL تعریف شده است.'}}
def safe(u):
...
def get(u,timeout=150):
 ...
 for n in range(5):
  try:
   r=urllib.request.Request(u,headers={'User-Agent':'GramissLegacyPantsBatch5/1.0','Cache-Control':'no-cache','Pragma':'no-cache'})
...
def call(mode,old=None):
 n='gramiss-pants-b5-'+mode+'-'+hashlib.sha256((mode+str(time.time())).encode()).hexdigest()[:12]+'.php';save(n,php(mode,old));s,b,f=get(BASE+'/'+n+'?t='+str(int(time.time())),240);d=json.loads(b.decode('utf-8','replace'));print('WP',mode,s,f,json.dumps(d,ensure_ascii=False,separators=(',',':')))
...
try:
 ...
 print('PASS LEGACY PANTS SHORT DESCRIPTION BATCH 5')
