...
OUT=Path(os.environ.get('PERF_OUT','/tmp/gramiss-perf-after-purge'))
...
def run(label,url,n):
    ...
    print('AFTER_PURGE_RUN',json.dumps({'label':label,'mode':'mobile','run':n,**row},sort_keys=True))
    return row

def main():
    plan=[('category_tshirt',2),('shop',1),('home',1)]
    summary={}
    for label,repeats in plan:
        rows=[run(label,TARGETS[label],i+1) for i in range(repeats)]
        after={k:med(rows,k) for k in rows[0].keys()}
        before=BEFORE[(label,'mobile')]
        delta={
        ...
        row={'label':label,'mode':'mobile','before':before,'after':after,'delta':delta}
        summary[f'{label}:mobile']=row
        print('BEFORE_AFTER_PURGE',json.dumps(row,sort_keys=True))
    ...
    if summary['home:mobile']['after']['total_bytes'] and summary['home:mobile']['after']['total_bytes'] > 3000000:
        raise SystemExit('FAIL home smoke weight regression')
    print('PASS PERFORMANCE AFTER CACHE PURGE V1 READ ONLY')
