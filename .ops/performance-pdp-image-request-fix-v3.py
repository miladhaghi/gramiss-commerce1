...
def api(fn,params,post=False):
 ...
 with urllib.request.urlopen(req,context=CTX,timeout=90) as x:p=json.loads(x.read().decode('utf-8','replace'))
...
def get(url,timeout=180):
 req=urllib.request.Request(safe_url(url),headers={'User-Agent':'GramissPDPImageFixV3/1.0','Cache-Control':'no-cache','Pragma':'no-cache'})
 with urllib.request.urlopen(req,context=CTX,timeout=timeout) as x:return x.status,x.read().decode('utf-8','replace')
def flush():
...
PDP_FILTERS="""<!-- GRAMISS PERFORMANCE PDP IMAGE REQUEST V1 START -->
<?php
add_filter( 'single_product_archive_thumbnail_size', function( $size ) { return 'gramiss-product-card'; }, 999 );
...
def verify():
 st,page=get(PDP+'?perf-image-v3='+str(time.time())); e=[]
 if st!=200:return ['PDP HTTP '+str(st)]
 if 'GRAMISS PERFORMANCE PDP IMAGE REQUEST V1 START' not in page:e.append('server image override missing')
 if 'product-runtime-gallery-fix.js?v=20260903-perf3' not in page:e.append('gallery cache bust missing')
 if 'product-mobile-v1-4.js?v=20260903-perf3' not in page:e.append('style cache bust missing')
 gallery=[]; related=[]
 ...
 if len(gallery)<3:e.append('gallery count <3')
  ...
  if '66vw' not in attr(gallery[0],'sizes'):e.append('main sizes not 66vw')
...
def main():
 ...
 for p,x in EXPECTED.items():
  a=sha(before[p]); print('BEFORE',p,a)
  if a!=x:raise SystemExit('REFUSE drift '+p)
 for p,x in PROTECTED.items():
  if sha(read(p))!=x:raise SystemExit('REFUSE protected drift '+p)
 h=before['header.php']; g=before['assets/js/product-runtime-gallery-fix.js']; s=before['assets/js/product-mobile-v1-4.js']
 if h.count('GRAMISS PERFORMANCE PDP IMAGE REQUEST V1 START')!=0:raise SystemExit('REFUSE override already present')
 nh=replace_once(h,LOADER,PDP_FILTERS+LOADER,'PDP loader insertion point')
 nh=sub_once(nh,r'product-runtime-gallery-fix\.js\?v=[^"\']+','product-runtime-gallery-fix.js?v=20260903-perf3','gallery loader')
 nh=sub_once(nh,r'product-mobile-v1-4\.js\?v=[^"\']+','product-mobile-v1-4.js?v=20260903-perf3','style loader')
 ng=replace_once(g,GALLERY_OLD,GALLERY_NEW,'gallery source'); ng=replace_once(ng,APPLY_OLD,APPLY_NEW,'gallery apply')
 ns=replace_once(s,STYLE_OLD,STYLE_NEW,'style source'); ns=replace_once(ns,CARD_OLD,CARD_NEW,'style card')
 changed={'header.php':nh,'assets/js/product-runtime-gallery-fix.js':ng,'assets/js/product-mobile-v1-4.js':ns}
 try:
  ...
  raise
 print('PASS PERFORMANCE PDP IMAGE REQUEST FIX V3')
