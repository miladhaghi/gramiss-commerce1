...
TARGET = 'inc/shop-premium-shell.php'
TARGET_SHA = '20b04bdd6fcfc7f7733a0ce0f661b387213b60ae26a6f69fb88dcdf904c8e2dc'
...
MARKER = 'GRAMISS_CATEGORY_SINGLE_H1_V1'
PROTECTED = {
    ...
    for attempt in range(4):
        try:
            req = urllib.request.Request(url, headers={
                'User-Agent': 'GramissCategorySingleH1RepairV1/1.0',
...
def inspect_category(url):
    status, raw, final = get(url + ('&' if '?' in url else '?') + 'g1h1=' + str(int(time.time() * 1000)), 180)
    text = raw.decode('utf-8', 'replace')
...
def sitemap(path):
    status, raw, _ = get(BASE + '/' + path + '?g1h1=' + str(int(time.time() * 1000)), 150)
...
def purge():
    ...
    name = 'gramiss-category-h1-purge-v1-' + nonce + '.php'
    php = r'''<?php
...
/* GRAMISS_CATEGORY_SINGLE_H1_V1
 * Keep the Gramiss premium category hero as the sole H1.
 * The native WooCommerce archive title was already visually hidden by the shop shell CSS;
 * suppress it at render time so it is also absent from the DOM/SEO heading structure.
 */
if ( ! function_exists( 'gramiss_category_single_h1_v1' ) ) {
    function gramiss_category_single_h1_v1( $show ) {
...
add_filter( 'woocommerce_show_page_title', 'gramiss_category_single_h1_v1', 999 );
'''
...
if MARKER not in old:
    ...
    if 'gramiss-premium-shop-title' not in old or 'gramiss-shop-premium-hero' not in old:
        raise SystemExit('FAIL TARGET MARKERS MISSING')
    pos = old.rfind('?>')
    new = old[:pos] + patch + '\n' + old[pos:] if pos >= 0 else old.rstrip() + patch + '\n'
...
print('PASS CATEGORY SINGLE H1 REPAIR V1', final_sha)
