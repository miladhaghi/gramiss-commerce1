...
EXPECTED={163:"تیشرت باکسی سنگشور طرح I'm Sorry با یقه‌گرد و طراحی چاپی، در رنگ‌های زرشکی و ذغالی و سایز L ارائه می‌شود.",167:'تیشرت باکس سنگشور طرح Me vs Me با یقه‌گرد و طراحی چاپی، در سایز L ارائه می‌شود.',197:'تیشرت کراپ باکس سنگشور طرح HORAE با یقه‌گرد و طراحی چاپی، در رنگ‌های کرم و ذغالی و سایزهای M و L ارائه می‌شود.',202:'تیشرت باکس سنگشور طرح Greedy با یقه‌گرد و طراحی چاپی، در رنگ‌های قهوه‌ای و ذغالی و سایزهای L و XL ارائه می‌شود.',215:'تیشرت باکس سنگشور طرح BackHouse با یقه‌گرد و طراحی چاپی، در سایزهای M و L ارائه می‌شود.'}
def safe(u):
...
def get(u,timeout=150):
 ...
 for n in range(5):
  try:
   r=urllib.request.Request(u,headers={'User-Agent':'GramissLegacyRemediationAuditV1/1.0','Cache-Control':'no-cache','Pragma':'no-cache'})
...
if HEALTHY and pre['front-page.php']!=HEALTHY:raise SystemExit('healthy home drift')
...
if cs!=200 or len(cu)!=20 or ch!=CAT_SHA:raise SystemExit('product category sitemap drift')
nonce=hashlib.sha256((str(time.time())+pre['front-page.php']).encode()).hexdigest()[:12];name='gramiss-remediation-audit-'+nonce+'.php'
php=r'''<?php header('Content-Type: application/json; charset=utf-8');define('WP_USE_THEMES',false);require __DIR__.'/wp-load.php';@unlink(__FILE__);$ids=get_posts(['post_type'=>'product','post_status'=>'publish','numberposts'=>-1,'orderby'=>'ID','order'=>'ASC','fields'=>'ids']);$o=['published'=>count($ids),'empty_excerpt'=>0,'rows'=>[]];foreach($ids as $id){$p=get_post($id);if(trim((string)$p->post_excerpt)==='')$o['empty_excerpt']++;if(in_array((int)$id,[163,167,197,202,215],true))$o['rows'][(string)$id]=['title'=>$p->post_title,'excerpt'=>$p->post_excerpt,'url'=>get_permalink($id),'robots'=>get_post_meta($id,'rank_math_robots',true)];}echo wp_json_encode($o,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);?>'''
save(name,php);st,b,_=get(BASE+'/'+name+'?t='+str(int(time.time())),180);d=json.loads(b.decode('utf-8','replace'));print('WP_STATE',json.dumps(d,ensure_ascii=False,separators=(',',':')))
errors=[]
if st!=200:errors.append('probe_http')
if d.get('published')!=48:errors.append('published_count')
if d.get('empty_excerpt')!=42:errors.append('empty_excerpt_expected_42')
for pid,exp in EXPECTED.items():
 row=(d.get('rows') or {}).get(str(pid));
 if not row:errors.append(f'{pid}:missing');continue
 if row.get('excerpt')!=exp:errors.append(f'{pid}:excerpt')
 st2,raw,_=get(row['url']+'?t='+str(int(time.time())),150);txt=raw.decode('utf-8','replace');head=txt.split('</head>',1)[0];desc=val(head,r'<meta[^>]+name=["\']description["\'][^>]+content=["\']([^"\']*)');canon=val(head,r'<link[^>]+rel=["\']canonical["\'][^>]+href=["\']([^"\']+)');robots=val(head,r'<meta[^>]+name=["\']robots["\'][^>]+content=["\']([^"\']+)');schemas=len(re.findall(r'"@type"\s*:\s*"Product"',txt,re.I));print('PRODUCT',pid,st2,'META',desc,'CANON',canon,'ROBOTS',robots,'SCHEMA',schemas)
 if st2!=200:errors.append(f'{pid}:http')
 if desc!=exp:errors.append(f'{pid}:meta')
 if norm(canon)!=norm(row['url']):errors.append(f'{pid}:canonical')
 if 'noindex' in robots.lower() or 'index' not in robots.lower():errors.append(f'{pid}:robots')
 if schemas!=1:errors.append(f'{pid}:schema')
post={p:hashlib.sha256(theme(p).encode()).hexdigest() for p in PROTECTED};print('PROTECTED_POST',json.dumps(post,sort_keys=True));
if post!=pre:errors.append('protected_changed')
print('ERRORS',json.dumps(errors,ensure_ascii=False));
if errors:raise SystemExit(1)
print('PASS LEGACY PRODUCT REMEDIATION AUDIT V1')