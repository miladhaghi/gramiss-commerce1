...
if s.count(old)!=1: raise SystemExit('v9 expected-anchor mismatch')
s=s.replace(old,new)
old_count="if d.get('empty_excerpt')!=24:errs.append('empty_excerpt')"
new_count="if d.get('empty_excerpt')!=9:errs.append('empty_excerpt')"
if s.count(old_count)!=1: raise SystemExit('v9 count-anchor mismatch')
s=s.replace(old_count,new_count)
s=s.replace("GramissLegacyRemediationAuditV5/1.0","GramissLegacyRemediationAuditV9/1.0")
s=s.replace("gramiss-remediation-audit-v5-","gramiss-remediation-audit-v9-")
s=s.replace("PASS LEGACY PRODUCT REMEDIATION AUDIT V5","PASS LEGACY PRODUCT REMEDIATION AUDIT V9")
exec(compile(s,'.ops/legacy-product-remediation-audit-v9.generated.py','exec'),{'__name__':'__main__'})
