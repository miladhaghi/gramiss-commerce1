...
TARGETS={359:{'title':'شلوار پارچه‌ای بگ ریزشی','excerpt':'شلوار پارچه‌ای بگ ریزشی؛ برای این مدل سایزهای M و L تعریف شده است.'},366:{'title':'شلوار پارچه‌ای فول بگ ریزشی','excerpt':'شلوار پارچه‌ای فول‌بگ ریزشی؛ برای این مدل سایزهای M، L، XL، 2XL و 3XL تعریف شده است.'}}
def safe(u):
...
def get(u,timeout=150):
 ...
 for n in range(5):
  try:
   r=urllib.request.Request(u,headers={'User-Agent':'GramissLegacyPantsBatch6/1.0','Cache-Control':'no-cache','Pragma':'no-cache'})
...
def call(mode,old=None):
 n='gramiss-pants-b6-'+mode+'-'+hashlib.sha256((mode+str(time.time())).encode()).hexdigest()[:12]+'.php';save(n,php(mode,old));s,b,f=get(BASE+'/'+n+'?t='+str(int(time.time())),240);d=json.loads(b.decode('utf-8','replace'));print('WP',mode,s,f,json.dumps(d,ensure_ascii=False,separators=(',',':')))
...
try:
 ...
 print('PASS LEGACY PANTS SHORT DESCRIPTION BATCH 6')
