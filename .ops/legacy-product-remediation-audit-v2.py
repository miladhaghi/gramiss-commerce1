...
def get(u,timeout=150):
 ...
 for n in range(5):
  try:
   r=urllib.request.Request(u,headers={'User-Agent':'GramissLegacyRemediationAuditV2/1.0','Cache-Control':'no-cache','Pragma':'no-cache'})
...
nonce=hashlib.sha256((str(time.time())+pre['front-page.php']).encode()).hexdigest()[:12];name='gramiss-remediation-audit-v2-'+nonce+'.php';ids=','.join(map(str,EXPECTED))
...
if d.get('empty_excerpt')!=37:errs.append('empty_excerpt')
...
print('PASS LEGACY PRODUCT REMEDIATION AUDIT V2')