...
save_file(f'header.php.bak-related-mobile-balance-{stamp}',header)
print('Header backup:',stamp)

css=r'''/* GRAMISS_PDP_RELATED_MOBILE_BALANCE_V1 */
/* Mobile-only: preserve the existing card footprint, visually enlarge product photography, and move text lower. */
...
start='<!-- GRAMISS PDP RELATED MOBILE BALANCE START -->'
end='<!-- GRAMISS PDP RELATED MOBILE BALANCE END -->'
block=start+'\n<link rel="stylesheet" id="gramiss-pdp-related-mobile-balance-css" href="<?php echo esc_url( get_stylesheet_directory_uri() . \'/assets/css/pdp-related-mobile-balance.css?v=20260817-1\' ); ?>">\n'+end
...
live_css=get_file('assets/css/pdp-related-mobile-balance.css')
checks={
    'mobile marker':'GRAMISS_PDP_RELATED_MOBILE_BALANCE_V1' in live_css,
    'mobile scope':'@media (max-width:767px)' in live_css,
    'image scale':'transform:scale(1.14)!important' in live_css,
    'white media':'background:#fff!important' in live_css,
    'title lower':'bottom:23px!important' in live_css,
    'price lower':'bottom:3px!important' in live_css,
    'header loader':'gramiss-pdp-related-mobile-balance-css' in live_header,
}
for label,ok in checks.items(): print(('PASS' if ok else 'FAIL')+': '+label)
if not all(checks.values()): raise SystemExit('Mobile related balance verification failed')
print('LIVE PDP RELATED MOBILE BALANCE DEPLOYED AND VERIFIED')
