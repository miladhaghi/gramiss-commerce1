...
ctx=ssl._create_unverified_context(); stamp=time.strftime('%Y%m%d-%H%M%S',time.gmtime()); version='20260827-2'
...
def public_get(url):
    req=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 GramissCartMobile/1.1','Cache-Control':'no-cache','Pragma':'no-cache','Accept':'*/*'})
...
write_at(theme,'header.php.bak-cart-mobile-v1-1-'+stamp,original); print('BACKUP header.php.bak-cart-mobile-v1-1-'+stamp)
...
checks={
 ...
 'cart guard page1':"is_page( 1 )" in live_h,
...
purge='gramiss-purge-cart-mobile-v11-'+stamp+'.php'; purge_php="<?php define('WP_USE_THEMES',false); require __DIR__.'/wp-load.php'; if(function_exists('do_action')){do_action('litespeed_purge_all');} echo 'OK'; @unlink(__FILE__);"
...
nonce=str(int(time.time())); st,b=public_get('https://gramiss.ir/?page_id=1&g1_cart_v11='+nonce); cart_html=b.decode('utf-8','replace')
public_checks={
 'cart page 200':st==200,
 'cart css ref':f'{css_rel}?v={version}' in cart_html,
 'cart js ref':f'{js_rel}?v={version}' in cart_html,
 'pdp assets absent on Cart':'product-mobile-v1-4.css' not in cart_html and 'product-mobile-v1-3.css' not in cart_html,
...
st,b=public_get('https://gramiss.ir/?p=392&g1_product_safety_cart_v11='+nonce); product_html=b.decode('utf-8','replace')
product_ok=st==200 and 'product-mobile-v1-4.css?v=20260827-5' in product_html and 'product-mobile-v1-4.js?v=20260827-5' in product_html and f'{css_rel}?v={version}' not in product_html and '">">' not in product_html
print(('PASS' if product_ok else 'FAIL')+': PDP preserved, normalized and Cart isolated')
if not product_ok: rollback('PDP safety verification failed')
st,b=public_get('https://gramiss.ir/?g1_home_safety_cart_v11='+nonce); home=b.decode('utf-8','replace'); home_ok=st==200 and 'g1-floating-hero' in home and 'data-g1-looks' in home and f'{css_rel}?v={version}' not in home and 'product-mobile-v1-4.css' not in home
print(('PASS' if home_ok else 'FAIL')+': Home/Looks clean')
if not home_ok: rollback('Home safety verification failed')
print('LIVE GRAMISS CART MOBILE V1.1 DEPLOYED')
