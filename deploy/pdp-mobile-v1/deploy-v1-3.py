...
css_rel='assets/css/product-mobile-v1-3.css'; js_rel='assets/js/product-mobile-v1-3.js'
css_path=Path('deploy/pdp-mobile-v1/product-mobile-v1-3.css'); js_path=Path('deploy/pdp-mobile-v1/product-mobile-v1-3.js')
css=css_path.read_text(encoding='utf-8'); js=js_path.read_text(encoding='utf-8')
if 'GRAMISS_PDP_MOBILE_STYLE_INTELLIGENCE_V1_3' not in css or 'GRAMISS_PDP_MOBILE_STYLE_INTELLIGENCE_V1_3' not in js: raise SystemExit('ABORT: V1.3 markers missing')
...
def public_get(url):
    req=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 GramissStyleLogic/1.3','Cache-Control':'no-cache','Pragma':'no-cache','Accept':'*/*'})
...
for needle in ('product-mobile-v1.css?v=20260827-1','product-mobile-v1.js?v=20260827-1','product-mobile-v1-1.css','product-mobile-v1-2.css','product-mobile-v1-2.js'):
    if needle not in header: raise SystemExit('ABORT: live PDP mobile baseline differs: '+needle)
css_link=f'<link id="gramiss-pdp-mobile-v1-3-css" rel="stylesheet" href="<?php echo esc_url( get_stylesheet_directory_uri() . \'/{css_rel}?v={version}\' ); ?>">'
js_link=f'<script id="gramiss-pdp-mobile-v1-3-js" src="<?php echo esc_url( get_stylesheet_directory_uri() . \'/{js_rel}?v={version}\' ); ?>" defer></script>'
for ident,markup in [('gramiss-pdp-mobile-v1-3-css',css_link),('gramiss-pdp-mobile-v1-3-js',js_link)]:
...
write_at(theme,'header.php.bak-pdp-mobile-v1-3-'+stamp,original); print('BACKUP header.php.bak-pdp-mobile-v1-3-'+stamp)
...
checks={'v1.3 css once':live_h.count('gramiss-pdp-mobile-v1-3-css')==1,'v1.3 js once':live_h.count('gramiss-pdp-mobile-v1-3-js')==1,'v1.3 css version':f'{css_rel}?v={version}' in live_h,'v1.3 js version':f'{js_rel}?v={version}' in live_h,'css exact':hashlib.sha256(live_css.encode()).hexdigest()==css_sha,'js exact':hashlib.sha256(live_js.encode()).hexdigest()==js_sha,'mobile guarded':'@media (max-width: 760px)' in live_css,'style module':'.g1-style-intelligence' in live_css and 'loadStylingProducts' in live_js}
...
purge='gramiss-purge-pdp-style-v13-'+stamp+'.php'; purge_php="<?php define('WP_USE_THEMES',false); require __DIR__.'/wp-load.php'; if(function_exists('do_action')){do_action('litespeed_purge_all');} echo 'OK'; @unlink(__FILE__);"
...
nonce=str(int(time.time())); st,b=public_get('https://gramiss.ir/?p=392&g1_pdp_style_v13='+nonce); html=b.decode('utf-8','replace')
public_checks={'product 200':st==200,'v1.3 css ref':f'{css_rel}?v={version}' in html,'v1.3 js ref':f'{js_rel}?v={version}' in html,'woo related section':'related products' in html.lower() or 'related products' in html,'woo variation form':'variations_form' in html,'v1.2 preserved':'product-mobile-v1-2.css' in html and 'product-mobile-v1-2.js' in html}
...
for rel,sha,marker in ((css_rel,css_sha,b'GRAMISS_PDP_MOBILE_STYLE_INTELLIGENCE_V1_3'),(js_rel,js_sha,b'GRAMISS_PDP_MOBILE_STYLE_INTELLIGENCE_V1_3')):
...
try:
    st,b=public_get('https://gramiss.ir/wp-json/wc/store/v1/products/categories?per_page=100'); cats=json.loads(b.decode('utf-8','replace')) if st==200 else []; print('STORE API',st,'categories',len(cats) if isinstance(cats,list) else 'n/a')
except Exception as exc: print('STORE API optional fallback:',exc)
st,b=public_get('https://gramiss.ir/?g1_home_safety_style_v13='+nonce); home=b.decode('utf-8','replace'); home_ok=st==200 and 'g1-floating-hero' in home and 'data-g1-looks' in home; print(('PASS' if home_ok else 'FAIL')+': Home/Looks untouched')
if not home_ok: rollback('Home safety verification failed')
print('LIVE PDP MOBILE STYLE INTELLIGENCE V1.3 DEPLOYED')
