...
old_css=get_file('assets/css/pdp-related-media-balance.css')
save_file(f'header.php.bak-related-media-v2-{stamp}',header)
save_file(f'assets/css/pdp-related-media-balance.css.bak-v2-{stamp}',old_css)
print('Backups:', stamp)

css=r'''/* GRAMISS_PDP_RELATED_MEDIA_BALANCE_V2 */
/* Desktop-only: preserve outer card footprint while giving more vertical space to the product image. */
...
block=start+'\n<link rel="stylesheet" id="gramiss-pdp-related-media-balance-css" href="<?php echo esc_url( get_stylesheet_directory_uri() . \'/assets/css/pdp-related-media-balance.css?v=20260817-2\' ); ?>">\n'+end
...
checks={
    'v2 marker':'GRAMISS_PDP_RELATED_MEDIA_BALANCE_V2' in live_css,
    'desktop scope':'@media (min-width:1101px)' in live_css,
    'taller image':'height:386px!important' in live_css,
    'tight image padding':'padding:2px 4px 0!important' in live_css,
    'same-footprint bottom reserve':'padding-bottom:60px!important' in live_css,
    'title lower':'bottom:24px!important' in live_css,
    'price lower':'bottom:4px!important' in live_css,
    'cache bust v2':'pdp-related-media-balance.css?v=20260817-2' in live_header,
}
for label,ok in checks.items(): print(('PASS' if ok else 'FAIL')+': '+label)
if not all(checks.values()): raise SystemExit('Related media balance v2 verification failed')
print('LIVE PDP RELATED CARD MEDIA BALANCE V2 DEPLOYED AND VERIFIED')
